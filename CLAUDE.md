...
**One seam to the thing that actually runs a turn: `internal/engine`.** `Controller` holds a single `engine.TurnEngine` — `Start(ctx, TurnRequest) (TurnHandle, error)`, and `TurnHandle` is `Cancel`/`Wait`. That is the entire boundary, and it is meant to stay that way: `internal/engine/boundary_test.go` fails if `TurnEngine` grows a second method, if any of the interfaces sprouts an application-policy verb (`Approve`, `History`, `Resume`, `Fork`, `SetPlanMode`, …), or if `internal/engine` or the dsh adapter imports `control`/`toolpolicy`/`permission`/`checkpoint`/`evidence`/`memory`/`billing`/`tool`/`plugin`. Everything OneCreat *is* — memory, evidence, permission, billing, checkpoints — sits above that seam and treats both engines alike. Two adapters implement it: `internal/engine/native` (wraps the `agent.Runner` — Agent or Coordinator) and `internal/engine/dsh` (the sidecar). `boot.selectEngine` picks one from `engine = "native"|"dsh"`, which is now a **live** config key — a bad value or an unlaunchable sidecar fails at assembly instead of silently running native. Engines differ, and they say so through capabilities rather than through extra methods: native declares `streaming`/`approval`/`resume`/`fork`, dsh declares only `streaming` (its tools run in its own process, and its session log — not OneCreat's — is the truth), and an engine that declares nothing supports nothing. Cancellation has exactly one trigger, the turn context; `TurnHandle.Cancel` is how that reaches an engine that would not die with it.
