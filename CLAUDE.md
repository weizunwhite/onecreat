...
**One seam to the thing that actually runs a turn: `internal/engine`.** `Controller` holds a single `engine.TurnEngine` — `Start(ctx, TurnRequest) (TurnHandle, error)`, and `TurnHandle` is `Cancel`/`Wait`. That is the entire boundary, and it is meant to stay that way: `internal/engine/boundary_test.go` fails if `TurnEngine` grows a second method, if any of the interfaces sprouts an application-policy verb (`Approve`, `History`, `Resume`, `Fork`, `SetPlanMode`, …), or if `internal/engine` or the dsh adapter imports `control`/`toolpolicy`/`permission`/`checkpoint`/`evidence`/`memory`/`billing`/`tool`/`plugin`. OneCreat's policies — memory, evidence, permission, billing, checkpoints — sit above that seam, but **they only reach an engine that runs tools on this side of it**, and that distinction is the whole safety story. `internal/engine/native` (wrapping the `agent.Runner`) declares `hosted-tools`: every call runs in this process, through the `toolpolicy` pipeline. `internal/engine/dsh` runs its own tools in its own process, so it cannot declare that one — but it declares `gated-tools`, which is the half that matters: dsh-tools' scheduler runs its `tools/pre-execute` waterfall **before dispatch**, our control-plane plugin awaits a Go-side verdict, and `boot.dshDecider` answers it by calling **the same `toolpolicy.Pipeline.Before`** the native path calls — plan mode, permission policy, interactive approval, PreToolUse hook, pre-edit checkpoint, in that order. A refusal means the tool never runs; a dropped channel or a cancelled turn means deny. So `boot.requireToolGating` **still fails closed**, on the gating *moment* rather than the execution *location*: an engine that declares neither `hosted-tools` nor `gated-tools` is refused at assembly with a typed `engine.UnsupportedError`, and there is deliberately no config switch to bypass it — a safety gate one line of TOML can disable is not a gate. `engine` is otherwise a live config key (a bad value fails at assembly instead of silently running native). Capabilities are **enforced, not just declared**: `Controller.Supports` is the read-only query, and `Fork`/`Branch`/`SwitchBranch`/`Rewind(conversation)`/`NewSession`/`Compact`/`Summarize`/`Resume` each call `requireCap` *before touching any state* — split by what the op actually needs: `CapResume` means "the engine can pick a session back up" (dsh can), `CapFork` means "OneCreat's own message log **is** the model-visible history" (dsh's is a projection, so compaction and conversation rewind are refused there; code rewind, being file-based checkpoints, is not), so an unsupported op can never leave a half-mutated shadow session behind a "success" the UI believes. An engine that declares nothing supports nothing. The session record carries the engine that actually ran it, never a hardcoded `native`. Cancellation has exactly one trigger, the turn context; `TurnHandle.Cancel` is how that reaches an engine that would not die with it.
...
**Not every event may be dropped.** `Kind.Durable()` (`internal/event`) classifies each kind: only `reasoning`, `text` and `tool_progress` are ephemeral — a later event supersedes them — and **everything else is durable by default**, so a new kind is state-bearing until proven otherwise. `internal/eventstream` is the one delivery implementation both SSE transports use: ephemeral frames are dropped when a subscriber falls behind, durable frames queue, and a client that will not consume them is disconnected (`stream_reset`) to re-sync rather than served a stream with invisible holes. `Publish` never blocks — it runs on the agent's run-loop goroutine. Every frame carries the V2 envelope (`schemaVersion`/`eventId`/`sequence`/`sessionId`/`tabId`/`timestamp`/`durable`) stamped by an `eventwire.Stamper`, one per stream; `sequence` is gap-free so a client can *detect* loss — and there is now somewhere to go once it does: `GET /snapshot` returns the authoritative state (transcript **plus** `running`, plan mode, pending approvals and pending asks — the things that appear in the stream exactly once), the stream opens with a named `stream_ready{streamId,sequence}` frame so a client knows where it joined, and `stream_reset` names `resyncFrom`. The bundled UI re-syncs on all three triggers (reset, reconnect, sequence gap) through one `resync()` path. Detecting a hole without a way to close it is what this used to be. `Encode` produces the payload only — envelopes belong to streams, not events.
...
- **SaaS gateway hides the backend model — never leak it.** In OneCreat's online deployment, the client talks to the platform AI gateway (an OpenAI-compatible endpoint) and the user only ever sees a subscription *tier* (标准/高级/旗舰). The real provider/model/route is a billing-and-routing secret: revealing it both leaks IP and lets users bypass tier-based metering. `config.ModelPrivacyPolicy` (`internal/config/config.go`) is injected at runtime to enforce this, and the client-side planner is disabled on the gateway path for the same reason. A provider client knows it is on the gateway from the explicit `provider.Config.Gateway` flag (never by matching a key-env name), and that flag is what stops an upstream error body from being surfaced verbatim. Don't add code paths or prompts that surface the underlying model name. Wallet/points readout lives in `internal/billing`.

- **The platform account is an object, not environment variables.** `internal/account` owns it: a `Gateway` (URL, bearer token, tier — with its own lock) and the `CredentialSource` a provider asks *on every request*, so a refreshed token reaches sessions that are already running without a rebuild. Whoever owns the account holds the `Gateway` and passes it down (`boot.Options.Gateway`, `control.Options.Gateway`); the desktop shares one object across every tab. `ONECREAT_GATEWAY_URL` / `_TOKEN` / `ONECREAT_TIER` survive only as a **transport**: `account.FromEnv()` imports them once at process start, `Gateway.Env()` projects a session into a subprocess. Reading or writing them anywhere else fails `internal/account/boundary_test.go`, which scans the whole tree.
...
- What is deliberately **not** workspace-scoped: the MCP host and jobs (session-owned), the skill/memory indexes (per-build snapshots — caching them would serve stale `AGENTS.md`), and the CodeGraph *binary path* (background auto-install must become visible next session, not "once every tab closes"). `runtime.Turn` **is** wired: `control.Options.Scope` carries the session scope, and every turn opens a `runtime.Turn` under it, so the turn's context is the scope's — closing a session cancels the turn in flight instead of relying on the Controller to remember, and cancelling a turn never touches its session. A Controller built without a `Scope` (the CLI and most tests) falls back to a `context.Background()`-derived turn, which is the pre-wiring behaviour; `TestTurnRootIsTheScopeNotBackground` fails if `Guarded` reaches for `context.Background()` directly again.

### Config & memory

- **Workspace is an explicit value, not the process cwd.** `internal/workspace.Context` (immutable, always absolute; zero value = process-cwd semantics) is threaded through `config.LoadIn`, `boot.Options.Workspace`, `builtin.ConfineWorkspace` and `control.Options.WorkspaceRoot`. Runtime workspace switching must never call `os.Chdir` — only *startup* may (`desktop.resolveStartupWorkspace`, the web `--workspace` flag). Anything workspace-relative (project config, `.mcp.json`, `.env`, memory, skills, file tools, bash, CodeGraph, LSP, hooks, checkpoint confinement) resolves against the Context. **`.env` never reaches the process environment**: `config.Env` is a per-workspace, read-only overlay (process env > this workspace's `.env` > `~/.env`), and every subprocess is handed `cfg.Env().Environ()` explicitly — bash (`ConfineWorkspace`), hooks (`hook.Runner.SetEnv`), MCP plugins (`plugin.Spec.BaseEnv`), language servers (`lsp.NewManagerWithEnv`). An `os.Setenv` here is what made two workspaces share one key; adding a new subprocess means threading that env to it, or the project's `.env` silently stops applying to it.
