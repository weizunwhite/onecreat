# OneCreat Desktop (Wails shell)

A native desktop window around the OneCreat Go kernel. The same
...
```sh
cd desktop
wails build          # → build/bin/OneCreat(.app/.exe)
```
...
- **macOS** — Gatekeeper may report the app "is damaged" or is from an
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/OneCreat.app
  ```
...
```sh
minisign -Vm OneCreat-darwin-arm64.zip \
