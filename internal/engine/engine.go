...
const (
	...
	// CapGatedTools:工具在**引擎自己的进程**里执行,但每一次调用在派发之前都会
	// 阻塞等待 OneCreat 的 `toolpolicy.Pipeline.Before` 裁决(权限门、plan mode
	// 只读门、PreToolUse hook、写前检查点),结果回来之后再经 `After` 记账。
	//
	// 与 CapHostedTools 的区别只有一个:**执行位置不同,门禁时点相同**。
	// hosted 是"工具就在 OneCreat 进程里跑",gated 是"工具在别处跑,但没有我们的
	// 放行它跑不起来"。安全上两者等价 —— 都不是"事后看一眼";把它们合成一条会让
	// 将来某个真的需要"工具在本地"的地方(比如共享进程内的文件句柄)踩空,
	// 所以拆成两条。
	//
	// 装配根的 fail-closed 判据因此是 `HostedTools || GatedTools`,见
	// `boot.selectEngine`。两条都没有 = 引擎在背着我们跑工具,拒绝装配。
	CapGatedTools Capability = "gated-tools"
)
...
// 顺序固定,便于稳定编码。
func All() []Capability {
	return []Capability{CapStreaming, CapApproval, CapResume, CapFork, CapHostedTools, CapGatedTools}
}
