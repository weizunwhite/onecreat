...
// ⚠️ 这是本轮重构在整个代码库里**唯一**新增的包级可变状态,所以在这里说清楚,
// 免得它成为"隐藏的全局状态"(那是本轮明令禁止的形态之一)。
//
// 它可以被接受的理由:
//   - 它镜像的是一个**已经存在**的全局事实 —— 进程环境。`.env` 的兼容导出写的就是
//     `os.Setenv`,这份记录只是让我们能读懂自己写下的东西;
//   - 只增不减、互斥保护,没有"当前值"这种会让人搞错的语义;
//   - 它的存在**完全依附于那个兼容导出**。哪天子进程环境也按工作区显式组装
//     (AR-R10 的后半),这两个变量应当一并删掉。
var (
