...
## dsh 引擎下的会话真源(engine = "dsh" 时)

Web 模式与桌面版共用同一套 Controller,所以切到 dsh 引擎时**前端一行不用改**。要知道的一件事是
**会话谁是真源**:

- 模型可见历史的真源 = **dsh 自己的 store**(JSONL,默认 zstd,落在 `<会话目录>/dsh/<工作区>/<会话id>/`);
- Go 侧 `agent.Session` 是**只读投影**:每轮结束把 user/assistant 文本追加进去,于是 History /
  会话文件 / 会话标题 / 前端恢复照旧工作。Go 侧**从不写回 dsh**,不存在双写;
- 两边靠会话 id 对上:dsh 会话 id = `oc-` + Go 会话文件路径的 sha1 前 24 位;
- 因此**重写消息日志**的操作(对话回退 / fork / 分支 / 手动压缩摘要)在 dsh 引擎下明确报
  "暂不支持" —— 改投影不改真源等于制造假象。文件级 rewind(代码回退)不受影响,照常可用。
